...
def update_clusters():
    ...
    for i in range(155):
        fname = glob.glob("/data/nlahaye/output/Learnergy/COP_WHALE_DEMO/cop_env_whales_" +\
             str(i) + ".zarr.clust.data_*clusters.zarr")[0]
        dat = zarr.load(fname)
    ...
    for key in movement_dfs.keys():
        #if "2017CA-Bmu-056" not in key and "2017CA-Bmu-05800" not in key:
        #    continue        
        with open(os.path.join(out_dir, "final_env_maps_simple_" + key + ".pkl"), "rb") as f:
        ...
        for i in range(len(movement_sub_df)):
            ...
            abstract_grid_sub[i] = ((np.array(abstract_grid_sub[i]) *100).astype(np.int16)).astype(np.float32) 
            print("HERE1", abstract_grid_sub[i].min(), abstract_grid_sub[i].max(), abstract_grid_sub[i].mean(), abstract_grid_sub[i].shape)
            #if abstract_grid_sub[i].max() > 0 and abstract_grid_sub[i].max() < 50: 
            ...
            #else:
            #abstract_grid_sub[i] = ((abstract_grid_sub[i]*100).astype(np.int16)).astype(np.float32) #*100
        ...
        for n in range(len(abstract_grid[key])):
            print("FINAL HERE", os.path.join(out_dir, "final_env_maps_simple_" + key + ".pkl"), abstract_grid[key][n].min(), abstract_grid[key][n].max())
   

        pkl_file = os.path.join(out_dir, "final_env_maps_simple_1_" + key + ".pkl")

...
def gen_scene_list():
    ...
    for i in range(155):
        print("/data/nlahaye/NatureNet_Env/Copernicus/cop_env_whales_" + str(i) + ".zarr")
        tmp = glob.glob("/data/nlahaye/NatureNet_Env/Copernicus/cop_env_whales_" + str(i) + ".zarr")
        if len(tmp) < 1:
    ...
    for uid in movement_dfs:
        ...
        for dind in range(len(movement_dfs_uid)):
            ...
            pkl_file = os.path.join(out_dir, "env_maps_fnames_whale" + uid + ".pkl")

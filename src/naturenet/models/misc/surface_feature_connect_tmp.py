        ...
        for instrument in yml_conf["instruments"]:
            ...
            if instrument not in pre_baked.keys() or (instrument in pre_baked.keys() and pre_baked[instrument]["final_data"] is None):
                ...
                for chn in range(data.shape[0]):
                    plt.imshow(np.squeeze(data[chn,:,:]))
                    plt.savefig(instrument + "_scene_" + str(i) + "_chan" + str(chn) + ".png")
                del scene
...
def run_surface_feature_connect(yml_conf, scenes_per_uid={}):
    ...
    uids = ['235283_6']    
    for uid in uids: #TODO movement_dfs:
        ...
        for dind in range(len(movement_dfs_uid)):
            ...
            for index, row in movement_df.iterrows():
                ...
                #TODO while st_str < df_st_str and scene_ind < len(prelim_scene_map["times"])-1:

...
def gen_grid_info(location, data, final_grid_res_deg, per_channel_stats, compute_final_grid_info = False):
    ...
    final_grid_steps = final_grid_res_deg / current_grid_res_deg

    width_mod = location.shape[1] - ((location[:,:,1].max() - location[:,:,1].min()) / final_grid_res_deg)
    height_mod = location.shape[0] - ((location[:,:,0].max() - location[:,:,0].min()) / final_grid_res_deg)
    print(((location[:,:,1].max() - location[:,:,1].min()) / final_grid_res_deg), ((location[:,:,0].max() - location[:,:,0].min()) / final_grid_res_deg))


    if width_mod > 0.0:
        width_mod = int(math.ceil(width_mod))
    else:
        width_mod = int(math.floor(width_mod))
  
    if height_mod > 0.0:
        height_mod = int(math.ceil(height_mod))
    else:
        height_mod = int(math.floor(height_mod))    


    print("GRID DIFFERENTIAL", current_grid_res_deg, location[0,1,0], location[0,0,0], final_grid_res_deg, final_grid_steps)
    ...
    if abs(width_mod) > 0.0:
        final_width = final_width + width_mod

    final_height = location.shape[0]
    #height_mod = int(math.ceil(location.shape[0] / final_grid_steps))
    if abs(height_mod) > 0.0:
        final_height = final_height + height_mod 

    print("HERE RESIZING ISSUE", location.shape, final_width, final_height, data.shape)
    ...
    if compute_final_grid_info:
        ...
        #TODO - decide if still needed and fix
        final_grid_steps = int(round(final_grid_steps))
...
def gen_prelim_scene_map(yml_conf, scene_count, per_channel_stats):
    ...
    ship = {"data" : None, "location" : None, "final_data":None} 

    pre_baked = { "Bathymetry" : bath, "Coastal_Dist" : coast, "Ship_Density": ship}
    ...
    for i in range(scene_count):
        ...
        for instrument in yml_conf["instruments"]:
            ...
            if instrument not in pre_baked.keys() or (instrument in pre_baked.keys() and pre_baked[instrument]["final_data"] is None):
                ...
                prelim_scene_map[instrument]["scenes"].append(data) #(final_data)
                if instrument in pre_baked:
                    pre_baked[instrument]["final_data"] = data #final_data
            else:
                prelim_scene_map[instrument]["scenes"].append(copy.deepcopy(pre_baked[instrument]["final_data"]))
                print("APPENDING DATA TO FINAL LIST", prelim_scene_map[instrument]["scenes"][-1].shape, pre_baked[instrument]["final_data"].shape)
...
def run_surface_feature_connect(yml_conf, scenes_per_uid={}):
    ...
    #movement_dfs = yml_conf["movement_dfs"]
    ...
    prelim_scene_fname = os.path.join(yml_conf["out_dir"], yml_conf["run_uid"] + "_prelim_scene_map.pkl")
    with open(prelim_scene_fname, 'wb') as f:
...
def run_surface_feature_connect_final(yml_conf, scenes_per_uid, clustering = None, scaler=None):
    ...
    if clustering is None:
        ...
        #Sample first 50 samples from each stream
        training_grids = None
        for uid in final_scenes_per_uid:
            for uid_ind in range(len(final_scenes_per_uid[uid])):
                final_ind = min(50, len(final_scenes_per_uid[uid][uid_ind]))
        ...
        scaler = MinMaxScaler()
        training_grids = scaler.fit_transform(training_grids)
    ...
    for uid in final_scenes_per_uid:
        for uid_ind in range(len(final_scenes_per_uid[uid])):
            for scene_ind in range(len(final_scenes_per_uid[uid][uid_ind])):
                ...
                grid_tmp = clustering.predict(scaler.transform(grid_tmp))
                grid_tmp = grid_tmp.reshape((init_shape[0], init_shape[1]))
                final_scenes_per_uid[uid][uid_ind][scene_ind] = grid_tmp

    return msrffr, final_scenes_per_uid, clustering, scaler
...
def build_and_save_envs(yml_fpath):
    ...
    if yml_conf["run_final_env_gen"]:
        ...
        if os.path.exists(os.path.join(yml_conf["out_dir"], "clust_env.joblib")):
            ...
            scaler = joblib.load(os.path.join(yml_conf["out_dir"], "clust_env_scaler.joblib"))

        msrffr, final_scenes_per_uid, clustering, scaler = run_surface_feature_connect_final(yml_conf, scenes_per_uid, clustering, scaler)
        ...
        if save_cluster:
            ...
            scaler_fname = os.path.join(yml_conf["out_dir"], "clust_env_scaler.joblib")

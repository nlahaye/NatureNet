...
def gen_scene_list():
    ...
    for i in range(398):
        print("/data/nlahaye/NatureNet_Env/Copernicus/cop_env_shark_" + str(i) + ".zarr")
        tmp = glob.glob("/data/nlahaye/NatureNet_Env/Copernicus/cop_env_shark_" + str(i) + ".zarr")
        if len(tmp) < 1:
    ...
    for uid in movement_dfs:
        ...
        for dind in range(len(movement_dfs_uid)):
            ...
            for index, row in movement_df.iterrows():
                ...
                #df_st_str = datetime.strptime(row['TimeValue'],"%Y-%m-%dT%H:%M:%SZ")  #row['date'][0:10],"%Y-%m-%d")
                print(df_st_str)
            ...
            pkl_file = os.path.join(out_dir, "env_maps_fnames_shark" + uid + ".pkl")

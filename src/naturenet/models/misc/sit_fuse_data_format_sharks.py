...
def gen_scene_list():
    ...
    tme = datetime.strptime("2023-11-01","%Y-%m-%d")
    for i in range(398):
        fname_glob = glob.glob("/data/nlahaye/output/Learnergy/COP_SHARK_DEMO/cop_env_shark_t" + str(i) + ".zarr.clust.data_*clusters.no_geo.tif")
        if len(fname_glob) < 1:
            continue
        fname = fname_glob[0]
        ...
        print("HERE DAT", i, dat.min(), dat.max(), np.unique(dat))

    
    df_dir = "/data/nlahaye/NatureNet/Hammerhead_Out_v1/"
    df_uid = "hammerhead_v1"
    out_dir = "/data/nlahaye/NatureNet/Hammerhead_Out_v1/"
    ...
    for uid in movement_dfs:
        ...
        for dind in range(len(movement_dfs_uid)):
            ...
            for index, row in movement_df.iterrows():
                ...
                df_st_str = datetime.strptime(row['date'][0:10],"%Y-%m-%d")
                ...
                if len(scenes_per_df) < act_index + 1:
                    ...
                    plt.imshow(scenes[scene_ind])
                    plt.savefig("ENV_REP_SHARK_" + uid + "_" + str(act_index) + "_" + str(scene_ind) + ".png")
                    plt.clf()
                else:
                    print("Inserting scene in ", uid, "at", act_index, "from", scene_ind)
                    scenes_per_df[act_index] = scenes[scene_ind]
                    plt.imshow(scenes[scene_ind])
                    plt.savefig("ENV_REP_SHARK_" + uid + "_" + str(act_index) + "_" + str(scene_ind) + ".png")
                    plt.clf()

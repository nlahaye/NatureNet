...
def plot_time_series(learnt_zs, env_maps, paths, f, n_hidden, out_dir, color_list, static_data, uids):
    ...
    for uid in movement_dfs:
        ...
        for dind in range(len(movement_dfs_uid)):
            ...
            if len(movement_dfs_uid[dind]) < 97: #TODO generalize - removing end of paths that weren't included in training/eval
                continue
            print(uid, dind, len(movement_dfs_uid[dind]), len(movement_dfs_uid[dind]) < 90, "ERRORS HERE")
            movement_df = movement_dfs_uid[dind]

            #dttm = movement_df.iloc[0]["date"]
            #if len(dttm) == 10:
            #    dttm = dttm + " 00:00:00"

            dttm = movement_df.iloc[0]['TimeValue']

            if min_dt is None:
                min_dt = datetime.strptime(dttm,"%Y-%m-%dT%H:%M:%SZ") #dttm, "%Y-%m-%d %H:%M:%S") #['TimeValue'],"%Y-%m-%dT%H:%M:%SZ")
            else:
                min_dt = min(min_dt, datetime.strptime(dttm,"%Y-%m-%dT%H:%M:%SZ")) #dttm, "%Y-%m-%d %H:%M:%S")) #['TimeValue'],"%Y-%m-%dT%H:%M:%SZ"))  
...
def run_plots(yml_conf):
    ...
    fname = str(n_hidden) + '_' + str(seed) + "_MLP_S2_net2.npz"

    fname = os.path.join(out_dir, "s2_complex", fname)
    ...
    fname = os.path.join(out_dir, "s2_complex", run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_naturenet_MLP_S2_new2_LL.npz") 
    params3 = jnp.load(fname, allow_pickle=True)
    jax_path_vmap=params3["learnt_zs"]

    fname = os.path.join(out_dir, "s2_complex", run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_naturenet_MLP_S2_new2_reward_filtered.npz")

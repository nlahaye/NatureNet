...
from hiql.algorithms import IAVI
...
def run_hiavi(yml_conf):
    ...
    output_dir = os.path.join(out_dir, 'iavi')
    ...
    for num_trajs in np.arange(0,5):
        for kf_idx, (train_idxes, test_idxes) in enumerate(kf.split(trajs)):
            ...
            agent = IAVI(num_states=n_states, num_actions=n_actions,
                         P=trans_prob, expert_policy=pi, discount=gamma)
            agent.train()

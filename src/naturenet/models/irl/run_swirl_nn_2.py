...
def run_swirl_nn(yml_conf):
    ...
    new_logpi02, new_log_Ps2, new_Rs2, new_R_state2, LL_list2 = em_train_jaxopt_netadam2(all_xohs2, all_xohs, all_aohs, logpi0_start, log_Ps_start, Rs_start, R_state, trans_probs,\
    ...
    new_logpi02, new_log_Ps2, new_Rs2, new_R_state2, LL_list2 = em_train_jaxopt_netadam2(all_xohs2, all_xohs, all_aohs, new_logpi02, new_log_Ps2, new_Rs2, new_R_state2, trans_probs,\
        new_trans_probs, 10)
    jnp.savez(os.path.join(out_dir, str(n_hidden) + '_' + str(seed) +' _MLP_S1_net2.npz'), new_logpi0=new_logpi02, new_log_Ps=new_log_Ps2, new_Rs=new_Rs2, new_R_state=new_R_state2.params, LL_list=LL_list2)
    ...
    fname = os.path.join(out_dir, run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_naturenet_MLP_S2_new2_reward_filtered.npz")
    jnp.savez(fname, reward_filtered=reward_m2_filtered)

    LL, train_LL, learnt_zs = learnt_LL22(new_logpi02, new_log_Ps2, new_Rs2, new_R_state2.params, R_state.apply_fn, trans_probs, all_aohs, all_xohs, all_xohs2)
    fname = os.path.join(out_dir, run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_naturenet_MLP_S2_new2_LL.npz")
    print(LL, train_LL)
    ...
    """ 
    # S-1
    new_logpi0, new_log_Ps, new_Rs, new_R_state, LL_list = em_train_jaxopt_netadam(all_xohs, all_aohs, logpi0_start, log_Ps_start, Rs_start, R_state2, trans_probs, 10, init=False, trans=False)
    ...
    new_logpi0 = jnp.array(new_logpi0, dtype=jnp.bfloat16)
    new_log_Ps = jnp.array(new_log_Ps, dtype=jnp.bfloat16)
    new_Rs = jnp.array(new_Rs, dtype=jnp.bfloat16)

    new_logpi0, new_log_Ps, new_Rs, new_R_state, LL_list = em_train_jaxopt_netadam(all_xohs, all_aohs, new_logpi0, new_log_Ps, new_Rs, new_R_state, trans_probs, 10)
    ...
    fname = os.path.join(out_dir, str(n_hidden) + '_' + str(seed) + "_MLP_S1_net2.npz")
    params2 = jnp.load(fname, allow_pickle=True)
    new_logpi02, new_log_Ps2, new_Rs2, new_R_state2, LL_list2 = params2['new_logpi0'], params2['new_log_Ps'], params2['new_Rs'], params2['new_R_state'], params2['LL_list']


    #TODO - change to all, train, test
    print("Final computations")
    reward_m1 = get_reward_m(trans_probs, new_R_state.params, R_state2.apply_fn)
    ...
    LL, train_LL, learnt_zs = learnt_LL21(new_logpi0, new_log_Ps, new_Rs, new_R_state.params, R_state2.apply_fn, trans_probs, all_aohs, all_xohs)    

...
from naturenet.models.irl.run_swirl import run_swirl_final
...
def loop_arhmm(yml_conf):
    ...
    for n_states in range(5,6): #yml_conf["min_latent"], yml_conf["max_latent"]):
        yml_conf["n_hidden"] = n_states
        yml_conf["n_hidden_init"] = n_states
        for i in range(9,10): #yml_conf["num_runs"]):

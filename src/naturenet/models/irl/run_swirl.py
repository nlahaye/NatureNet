...
def run_swirl_init(yml_conf):
    ...
    new_logpi0, new_log_Ps, new_Rs, new_reward, LL_list = em_train_temp(jnp.array(new_logpi0), jnp.array(new_log_Ps), new_Rs, jnp.array(new_reward), temps, trans_prob, all_xohs, all_aohs, 100)
...
def run_swirl_final(yml_conf):
    ...
    time_interval_learnt_params = jnp.load(os.path.join(out_dir,time_interval_params), allow_pickle=True)
    ...
    new_logpi02, new_log_Ps2, new_Rs2, new_reward2, LL_list2 = em_train2_naturenet(jnp.array(logpi0_start), jnp.array(log_Ps_start), Rs_start, jnp.array(R_start)[:, None].reshape(n_hidden, emission_dim, n_states*n_actions),\
         new_trans_prob, all_xohs, all_xohs2, all_aohs, 50)
    fname = run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_naturenet_iter2_init.npz" 
    fname = os.path.join(out_dir, fname) 
    ...
    new_logpi02, new_log_Ps2, new_Rs2, new_reward2, LL_list2 = em_train2_temp(jnp.array(new_logpi02), jnp.array(new_log_Ps2), new_Rs2, jnp.array(new_reward2), temps, new_trans_prob, all_xohs, all_xohs2, all_aohs, 50)
    ...
    new_logpi0, new_log_Ps, new_Rs, new_reward, LL_list = em_train_naturenet(jnp.array(logpi0_start), jnp.array(log_Ps_start), Rs_start, jnp.array(R_start2)[:, None],\
         trans_prob, all_xohs, all_aohs, 50)
    fname = run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_naturenet_iter2_temp_init.npz"
    fname = os.path.join(out_dir, fname)
    jnp.savez(fname, new_logpi0=new_logpi0, new_log_Ps=new_log_Ps, new_Rs=np.array(new_Rs, dtype=object), new_reward=new_reward, LL_list=LL_list, temps=temps)

    print("Training third model step 2")
    new_logpi0, new_log_Ps, new_Rs, new_reward, LL_list = em_train_temp(jnp.array(new_logpi0), jnp.array(new_log_Ps), new_Rs, jnp.array(new_reward), temps, trans_prob, all_xohs, all_aohs, 50)
    fname = run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_naturenet_iter2_temp_2.npz"
    fname = os.path.join(out_dir, fname)
    jnp.savez(fname, new_logpi0=new_logpi0, new_log_Ps=new_log_Ps, new_Rs=np.array(new_Rs, dtype=object), new_reward=new_reward, LL_list=LL_list, temps=temps)
    ...
    LL, train_LL, test_LL, jax_path_vmap = learnt_LL2(new_logpi02, new_log_Ps2, new_Rs2, new_reward2, temps, n_states, n_actions, new_trans_prob, all_xohs, all_xohs2, all_aohs, all_xohs, all_xohs2, all_aohs, all_xohs, all_xohs2, all_aohs)

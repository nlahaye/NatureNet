...
def run_swirl_init(yml_conf):
    ...
    for j in range(len(positions)):
        if len(positions[j]) == max_len:
            ...
            print(len(positions_final), len(actions_final), j, len(positions[j]), len(actions[j]))
    ...
    epochs = 50 #50
    epochs_2 = 100 #100
    print(all_xohs.shape, prev_state_map.keys(), n_actions, n_states, len(Rs_start), len(R_start2))
    new_logpi0, new_log_Ps, new_Rs, new_reward, LL_list = em_train_temp(jnp.array(logpi0_start), jnp.array(log_Ps_start), Rs_start, jnp.array(R_start2)[:, None], temps, trans_prob, all_xohs, all_aohs, epochs, init=False, trans=False)


    fname = run_uid + "_swirl_run_" + str(n_hidden_init) + '_' + str(seed) + "_naturenet_init0.npz"
    ...
    new_logpi0, new_log_Ps, new_Rs, new_reward, LL_list = em_train_temp(jnp.array(new_logpi0), jnp.array(new_log_Ps), new_Rs, jnp.array(new_reward), temps, trans_prob, all_xohs, all_aohs, epochs_2)

    new_reward = normalize_reward(new_reward)
    print(new_reward.shape)
    if new_reward.ndim > 3:
        new_reward = new_reward[[1, 0, 2], ...] #TODO - viz this
    else:
        new_reward = new_reward[[1, 0], ...]
    fname = run_uid + "_swirl_run_" + str(n_hidden_init) + '_' + str(seed) + "_naturenet_init1.npz"
    fname = os.path.join(out_dir, fname)
    jnp.savez(fname, new_logpi0=new_logpi0, new_log_Ps=new_log_Ps, new_Rs=np.array(new_Rs, dtype=object), new_reward=new_reward, LL_list=LL_list, temps=temps)
...
def run_swirl_final(yml_conf):
    ...
    run_uid = yml_conf["run_uid"]
    arhmm_s_fname = os.path.join(out_dir, run_uid + "_" + str(n_hidden_init) + "_hidden_" + str(seed) + '_seed_arhmm_s.npz')
    swirl_params = run_uid + "_swirl_run_" + str(n_hidden_init) + '_' + str(seed) + "_naturenet_init1.npz"
    ...
    train_xohs = all_xohs[:]
    train_xohs2 = all_xohs2[:]
    train_xs_prev = all_xs_prev[:]
    train_xohs_prev = all_xohs_prev[:] 

    test_aohs = all_aohs[-5:]
    test_xohs = all_xohs[-5:]
    test_xohs2 = all_xohs2[-5:]
    test_xs_prev = all_xs_prev[-5:]
    test_xohs_prev = all_xohs_prev[-5:]
    ...
    fname = run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_S2_naturenet_iter2_init.npz" 
    fname = os.path.join(out_dir, fname) 
    ...
    new_logpi02, new_log_Ps2, new_Rs2, new_reward2, LL_list2 = em_train2_temp(jnp.array(new_logpi02), jnp.array(new_log_Ps2), new_Rs2, jnp.array(new_reward2), temps, new_trans_prob, train_xohs, train_xohs2, train_aohs, epochs)
    ...
    fname = run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_S1_naturenet_iter2_temp_init.npz"
    fname = os.path.join(out_dir, fname)
    jnp.savez(fname, new_logpi0=new_logpi0, new_log_Ps=new_log_Ps, new_Rs=np.array(new_Rs, dtype=object), new_reward=new_reward, LL_list=LL_list, temps=temps)

    print("Training third model step 2")
    new_logpi0, new_log_Ps, new_Rs, new_reward, LL_list = em_train_temp(jnp.array(new_logpi0), jnp.array(new_log_Ps), new_Rs, jnp.array(new_reward), temps, trans_prob, train_xohs, train_aohs, epochs)
    fname = run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_S1_naturenet_iter2_temp_2.npz"
    fname = os.path.join(out_dir, fname)
    jnp.savez(fname, new_logpi0=new_logpi0, new_log_Ps=new_log_Ps, new_Rs=np.array(new_Rs, dtype=object), new_reward=new_reward, LL_list=LL_list, temps=temps)
    ...
    print(LL)
    fname = os.path.join(out_dir, run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_S1_naturenet_iter2_LL1.npz") 
    jnp.savez(fname, LL=LL, jax_path_vmap=jax_path_vmap)

    print("Final computations, test", "S1")
    ...
    print(LL)
    fname = os.path.join(out_dir, run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_S1_naturenet_iter2_test_LL1.npz")
    jnp.savez(fname, LL=LL, jax_path_vmap=jax_path_vmap)
    ...
    print("Final computations, test", "S2")
    ...
    print(LL)
    fname = os.path.join(out_dir, run_uid + "_" + str(n_hidden) + '_' + str(seed) + "_S2_naturenet_iter2_test)LL2.npz")
    jnp.savez(fname, LL=LL, jax_path_vmap=jax_path_vmap)

...
def run_swirl_init(yml_conf):
    ...
    actions = np.load(actions_fpath)
    positions = np.load(positions_fpath)
    ...
    all_xs_prev = preprocess_xs_prev_np(positions[:, 1:], positions[:, :-1], prev_state_map)
...
def run_swirl_final(yml_conf):
    ...
    arhmm_s_params = np.load(arhmm_s_fname)
    ...
    # S-2
    new_logpi02, new_log_Ps2, new_Rs2, new_reward2, LL_list2 = em_train2_labyrinth(jnp.array(logpi0_start), jnp.array(log_Ps_start), Rs_start, jnp.array(R_start)[:, None].reshape(n_hidden, emission_dim, n_states*n_actions), 50)
    ...
    LL, train_LL, test_LL, jax_path_vmap = learnt_LL2(new_logpi02, new_log_Ps2, new_Rs2, new_reward2, temps, n_states, n_actions, trans_probs, all_xohs, all_aohs, all_xohs, all_aohs, all_aohs, all_xohs)
